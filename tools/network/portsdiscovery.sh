...
: <<'DESCRIPTION'
portsdiscovery.sh - Script para descubrir hosts activos en una red local, los que responden al ping.

Este script detecta automáticamente la IP y la máscara de subred de la interfaz de red activa,
calcula el rango de direcciones IP disponibles y realiza un escaneo de puertos comunes. 
(Para cuando los hosts no responden al ping) 
...
for i in $(seq 1 $hosts); do
    for port in 21 22 23 25 80 139 443 445 8080; do
		timeout 1 bash -c "echo '' > /dev/tcp/${network_base}.$i/$port" 2>/dev/null && echo "[+] Host ${network_base}.$i - Port $port (OPEN)" &
	done
done
